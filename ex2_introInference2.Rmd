...
ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
  ...
  scale_y_continuous(breaks = NULL) +
        ...
        annotate("text", x = 0.8, y = 0.15, label = "The observed t", angle = -90) +
...
As the area under the curve in the blue area is `r round(p, 2)`% of the distribution we may make a statement such as. "If the null-hypothesis is true, our observed value, or value even more distant from 0 would appear `r round(p, 2)`% of the times upon repeated sampling".
...
The above code calculates the proportion of mean differences that occurred when we divided the group by chance. 
